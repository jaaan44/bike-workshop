...
## 28. Phase 11C — Deployment Hardening & Operations Cleanup

Phase 11B (staging deployment + live validation, using Phase 11A's runbook) has since **completed successfully**: `https://bikeworkshop.storm-ark.com` is live, release `3d37101a3ba2ad0b6ecfce222979cc982d667c5a` on branch `claude/bicycle-workshop-app-6mp087`, full customer/staff workflow manually verified. That real deployment surfaced three genuine deployment-tooling weaknesses and several documentation gaps; Phase 11C (2026-09-21) fixes them. **Repository-only phase**: no live/staging VPS access, no SSH, no application business logic change — see `docs/DEPLOYMENT.md` §0 for the full write-up this section summarizes.

### 28.1 Root cause: stale `public/build`/`vendor` named volumes

`compose.yaml` shadows `vendor/` and `public/build/` (subpaths hidden by the full-repo bind mount, since both are gitignored) with named volumes so the image's build output survives the bind mount. **Docker seeds a named volume from image content only the first time it's used** — a later `docker compose build && docker compose up -d` does not refresh an already-populated volume. During the real deployment, the rebuilt image's new hashed Vite assets sat unused behind the pre-existing `bicycle_workshop_build` volume until it was manually removed and recreated. `vendor/` is exposed to the identical structural risk (confirmed by reading the same mount configuration) — it simply hadn't been triggered yet, since that release didn't change `composer.lock`.

**Fix:** `Dockerfile` now also copies its own `vendor/`+`public/build/` to a pristine, unmounted `/opt/release-artifacts/` at build time, tagged with a checksum of what determines each artifact's content (`composer.lock`; `public/build/manifest.json`). `docker/entrypoint.sh` compares that checksum against a marker it maintains inside each named volume on every container start and refreshes the volume from the pristine copy whenever they differ. This closes the bug for both `vendor/` and `public/build/` without changing local-dev's bind-mount workflow and without touching the MySQL data volume.

### 28.2 Network exposure hardened at the repository level

The desired staging posture (app reachable only via `127.0.0.1:8013`; MySQL not host-published) previously required an uncommitted, permanent edit to `compose.yaml` directly on the VPS. `compose.yaml`'s own defaults are now that posture; a new git-tracked `compose.override.yaml` restores the local-dev convenience of a host-published MySQL port (Compose auto-merges it for a plain `docker compose up`). Staging/production sets one `.env` line, `COMPOSE_FILE=compose.yaml`, to ignore that override — no more uncommitted `compose.yaml` edits on the server, ever. Validated with `docker compose config` under both the default (dev) and `COMPOSE_FILE=compose.yaml` (staging) cases.

### 28.3 Documentation brought in line with the real deployment

`docs/DEPLOYMENT.md` was substantially updated (a new §0 summarizing all Phase 11C changes, plus targeted updates throughout) to: document the real confirmed request path (Cloudflare → host Nginx :443 → `127.0.0.1:8013` → container Apache → Laravel); replace resolved "[Phase 11B — verify on server]" placeholders with the real, confirmed values from the completed deployment (§32 is now a historical record, not a to-do list); correct the database backup procedure to `mysqldump --no-tablespaces` using the app's own DB user rather than granting it `PROCESS`; add an explicit `.env`-backup step; call out the `APP_URL` correction as its own deployment-sequence step; document how to review/rotate the legacy `staff@example.com` staging account manually (§9.1) — **no code was added to auto-delete users**; record this release's rollback baseline (`PREVIOUS_GOOD_COMMIT=60847e9e5d6ef6cdf6a2ef9c272bf04efad36f95`); and conclude, with reasoning, that the Apache `AH00558` `ServerName` warning is cosmetic and left unchanged.

### 28.4 Migration-on-boot: audited, retained

Re-examined whether `docker/entrypoint.sh` should keep auto-running `php artisan migrate --force` on every `app` container start. **Retained, unchanged** — migrations here are additive-only, the maintenance-mode-first/backup-first sequence already protects against the real risk, and removing the automatic step would trade simplicity for a manual gate that adds an easy-to-forget step without removing the need for that same backup/maintenance discipline. The consequence — any `app` container start can migrate whatever database the active `.env` points to, with no confirmation prompt — is now stated explicitly in `docs/DEPLOYMENT.md` §30 as an operator rule, not left implicit.

### 28.5 Validation performed

- `php artisan test` → **161 tests, 442 assertions, 0 failures** — unchanged from the Phase 10B/11A baseline, confirming the Docker/entrypoint/compose changes touched no application behavior.
- `./vendor/bin/pint --test` → clean.
- `npm run build` → succeeds, producing `public/build/manifest.json` + hashed assets.
- `docker compose config` → valid under both the default local-dev merge (app `127.0.0.1:8013` + mysql `3348`) and `COMPOSE_FILE=compose.yaml` (app `127.0.0.1:8013` only, no `mysql` `ports:` key at all).
- `docker compose build`/`up` could **not** be executed end-to-end in this environment (no Docker daemon available here, same limitation Phase 11A's audit noted) — the `Dockerfile`/`docker/entrypoint.sh` changes were reviewed line-by-line and validated with every check that doesn't require a running daemon or registry access. **This phase did not deploy anything to the staging VPS.**

### 28.6 What Phase 11C deliberately did not do

No Kubernetes, CI/CD, Redis, queue workers, or microservices were introduced (none were needed — the existing two-service Compose setup already covers this app's actual requirements). No application business logic, route, view, or migration changed. No user was deleted from any database, and no code to do so was written. The live staging VPS was not accessed, modified, or SSHed into. Phase 10's deferred product/code backlog (§26.2) and the log-rotation `.env` recommendation (§19/§27.2) remain exactly as deferred — none of them are deployment-tooling defects this phase's scope covers.

### 28.7 Recommended next phase

With deployment tooling now hardened and documented against real production experience, the natural next phase is **product/code hardening from the existing backlog** (§26.2/HANDOFF.md "What should be implemented next?") — e.g. per-technician authorization, bicycle delete/archive, or the `scheduled`-status cleanup — rather than further deployment work, since Phase 11C closes out every deployment weakness the real staging deployment actually surfaced. If host-level items remain open (firewall confirmation, log rotation — `docs/DEPLOYMENT.md` §32), those are lightweight, code-free VPS/ops tasks that can happen independently of any future code phase. **Automated backup scheduling and restore-testing were completed and verified separately, on 2026-09-21** — see `docs/DEPLOYMENT.md` §34.

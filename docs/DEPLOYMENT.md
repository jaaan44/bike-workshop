...
**Phase:** 11C — Deployment Hardening & Operations Cleanup, following the completed real staging deployment, plus a Phase 11C follow-up (§0.5) hardening `docker/entrypoint.sh` against stale Laravel bootstrap caches.
...
**Fix (this follow-up):** `docker/entrypoint.sh` now deletes every generated `bootstrap/cache/*.php` file (`config.php`, `routes-v7.php`, `events.php`, `services.php`, `packages.php` — the exact five names `bootstrap/cache/.gitignore` wildcards away from git, confirmed by reading it; nothing else is source-controlled or build-time-generated there) **unconditionally, at the start of every container boot, before any `php artisan` command runs** — not just before `migrate --force`, but before the `key:generate` check too. The old post-migrate `config:clear` call is removed as redundant: by the time execution reaches where it used to run, no cache file has existed for the entire boot. See `docker/entrypoint.sh`'s own comment above `clear_stale_bootstrap_cache()` for the full reasoning, and `docker/entrypoint.test.sh` for a regression test.

**Why this is safe and why the other four bootstrap/cache files are included too:**
- All five are `rm -f`'d, which is a silent no-op when a file doesn't exist — safe on a first-ever boot (no cache files yet), an ordinary restart (`.env` unchanged, nothing was cached), and this exact incident (`.env` changed, a cache file exists) alike. Idempotent by construction (requirement: startup must be idempotent).
- None are source-controlled (`bootstrap/cache/.gitignore` is `*` / `!.gitignore`) and none are written by the `Dockerfile` at image-build time — every one of them only exists because *something* generated it against filesystem/`.env` state that may since have changed, so deleting them can never lose anything that isn't trivially regenerable.
- `config.php` is the one that caused this incident, but `routes-v7.php` (`route:cache`) and `events.php` (`event:cache`) are exactly as capable of masking a stale `.env`/code state after a manual optimization step, for the same structural reason. `services.php`/`packages.php` (package-discovery manifest, normally written by Composer's `post-autoload-dump` hook) aren't `.env`-derived, but they're keyed to whatever is currently in `vendor/` — and `vendor/` is itself swapped out from underneath `bootstrap/cache/` by the Phase 11C checksum-refresh mechanism (§0.1/§0.2) whenever the image's `composer.lock` changes. A discovery manifest left over from a previous `vendor/` contents is the same class of bug (stale generated cache overriding live state) even though the trigger differs, so it's cleared for the same reason, not as unrelated cleanup.
...
**File ownership — no host-side `sudo` dependency:** during the real incident, recovery required deleting `bootstrap/cache/config.php` by hand, but it (and its sibling files) were `www-data`-owned from a previous container's `chown -R www-data:www-data storage bootstrap/cache` (`docker/entrypoint.sh`, unchanged by this fix), and the deploy user lacked permission/`sudo` to remove them directly on the host. The container itself always runs as **root** (no `USER` directive in the `Dockerfile`, confirmed by re-reading it) — root can delete a `www-data`-owned file inside its own container (and, because of the bind mount, on the host filesystem too) unconditionally. Because the new `clear_stale_bootstrap_cache` step now runs automatically, as root, on every boot, an operator should never again need to manually delete these files — with or without `sudo` — to recover from this class of failure.

**Consequence for the optional Laravel optimization caches (§12/§28 step 11/§33):** those commands remain valid and safe to run manually after a deployment for a minor performance benefit, but the cache they produce is now **ephemeral, not persistent** — every subsequent container start (not just one after an `.env` change) clears it again. An operator who wants the optimization caches active must re-run them after every `docker compose up -d`/restart, not just once after a deploy. This is a deliberate safety-over-performance tradeoff: it guarantees requirement "startup always uses the current `.env`" unconditionally, at the cost of the optimization needing to be reapplied more often than before. Given this app's modest expected traffic (a single small workshop, per §12's own framing), this tradeoff is judged worth it — see §12/§28/§33 for the updated guidance.
...
- **This is also why `docker/entrypoint.sh`'s `clear_stale_bootstrap_cache` step (§0.5) never depends on the host deploy user having `sudo`:** it runs as root, before the `chown` above, so it can delete `www-data`-owned generated files inside `bootstrap/cache/` regardless of who owns them — exactly the permission problem that blocked manual recovery during the real credential-rotation incident, now handled automatically on every boot instead of requiring host-side `sudo rm`.
...
11. **(Optional) Apply the Laravel optimization caches** (§12, verified safe in this audit): `docker compose exec app php artisan config:cache && docker compose exec app php artisan route:cache && docker compose exec app php artisan view:cache`. **Note (§0.5):** `docker/entrypoint.sh` clears these again on every container start, including a plain restart — this step must be repeated after each `docker compose up -d`/restart, not just once after a deploy, to keep the optimization active.
...
docker compose exec app php artisan route:cache    # optional — same caveat
docker compose exec app php artisan view:cache     # optional — same caveat
...
`event:cache` is intentionally omitted (§12 — safe but a no-op today). `--no-dev` is intentionally not part of the Composer step (§14 — it happens inside `docker compose build`, not as a separate manual command, and is deliberately not used here). `storage:link` is intentionally omitted (§6 — not required). The three optional cache commands above never need to be run to keep the app *correct* — `docker/entrypoint.sh` itself now guarantees a fresh `.env`/config read on every start regardless (§0.5) — they only affect a minor performance optimization, and must be re-run after every subsequent restart to stay in effect.
